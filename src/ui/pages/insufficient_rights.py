...
class InsufficientRightsPage(BaseDetailPage):
    def __init__(self, parent=None):
        super().__init__("Недостаточно прав", parent)
        self.setup_ui()
    ...
    def _restore_selected(self):
        ...
        if dialog.exec():
            ...
            self.db.restore_insufficient_rights(ids)
            self.load_data()
    ...
    def _delete_selected(self):
        ...
        if dialog.exec():
            ids = [acc["id"] for acc in selected]
            self.db.delete_insufficient_rights(ids)
            self.load_data()
            main_window = self.window()
            if hasattr(main_window, 'accounts_page'):
                main_window.accounts_page._update_stats()
    ...
    def load_data(self):
        rows = self.db.get_insufficient_rights()
        self.all_accounts = []
        for acc in rows:
        ...
        self.stats_bar.update_card_count("Недостаточно прав", len(self.all_accounts))
