...
from src.workers.dm_broadcast import DMBroadcastWorker
...
class ActionsPage(QWidget):
    ...
    def _create_actions_menu(self):
        ...
        self.dm_action = QAction("Рассылка по ЛС", self)
        self.dm_action.triggered.connect(lambda: self._select_action("dm", "Рассылка по ЛС"))
        self.actions_menu.addAction(self.dm_action)
    ...
    def _select_action(self, action_type, action_name):
        if self.current_action == action_type:
            ...
            self.dm_action.setText("Рассылка по ЛС")
            ...
            self.dm_action.setText("• Рассылка по ЛС" if action_type == "dm" else "Рассылка по ЛС")
    ...
    def _start_broadcast(self):
        ...
        if self.current_action == "dm":
            self.broadcast = DMBroadcastWorker(
                self.selected_accounts, self.selected_proxies,
                message, attachments, max_workers, delay, parent=self
            )
            self.add_log(f"ЛС рассылка запущена: {len(self.selected_accounts)} аккаунтов", "blue")
        else:
    ...
    def _format_status_text(self, data: dict) -> tuple:
        ...
        if 'users' in data:
            text = (
                f"{data['token']} | Пользователей: {data['users']} | "
