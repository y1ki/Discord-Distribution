...
class Database:
    ...
    def get_sent_dms(self, token: str) -> set:
        with self.lock:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT user_id FROM dm_history WHERE token = ?", (token,))
            result = {row[0] for row in cursor.fetchall()}
            conn.close()
            return result

    def save_sent_dm(self, token: str, user_id: str):
        with self.lock:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute(
                "INSERT OR IGNORE INTO dm_history (token, user_id) VALUES (?, ?)",
                (token, user_id)
            )
            conn.commit()
            conn.close()

    def clear_dm_history(self):
        with self.lock:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM dm_history")
            conn.commit()
            conn.close()
